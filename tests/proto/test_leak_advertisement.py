...
class TestLeakManufacturerData:
    ...
    def test_roundtrip(self):
        for adv in (
            self._mk(),
            self._mk(alarm_on_wet=False, wet=False, audible=False, beat_state=False),
    ...
    def test_parses_known_frame(self):
        ...
        # alarm_on_wet(7)=1, wet(6)=1, audible(5)=1, volume(4:3)=2,
        ...
        assert adv.alarm_on_wet is True
        assert adv.wet is True
        assert adv.audible is True
    ...
    def test_byte8_bit_positions(self):
        ...
        assert self._mk(alarm_on_wet=True).to_bytes()[8] & 0x80
        assert self._mk(wet=True).to_bytes()[8] & 0x40
        assert self._mk(audible=True).to_bytes()[8] & 0x20
